...
pub struct Socket<'a> {
    ...
    /// Optional cap on receive-window credit, independent of physical storage.
...
impl<'a> Socket<'a> {
        ...
        T: Into<SocketBuffer<'a>>,
    {
        ...
        let remote_win_shift = Self::window_scale_for(rx_capacity);
    ...
    fn scaled_window(&self) -> u16 {
        cmp::min(
            self.receive_window() >> self.remote_win_shift as usize,
            (1 << 16) - 1,
        ) as u16
    }

    #[inline]
    fn window_scale_for(window: usize) -> u8 {
        let window_log2 = mem::size_of::<usize>() * 8 - window.leading_zeros() as usize;
        window_log2.saturating_sub(16) as u8
    }

    #[inline]
    fn max_receive_window(&self) -> usize {
        self.receive_window_limit
            .unwrap_or(self.rx_buffer.capacity())
            .min(self.rx_buffer.capacity())
    }
    ...
    /// Return the configured receive-window credit limit, if any.
    ///
    /// This is independent of [`Self::recv_capacity`], which continues to
    /// report physical receive-buffer storage.
    pub fn receive_window_limit(&self) -> Option<usize> {
        self.receive_window_limit
    }

    /// Limit receive-window credit without reducing physical buffer storage.
    ///
    /// Call this before `listen` or `connect` so window scaling is negotiated
    /// from the limit. Calling it on an active socket still bounds credit with
    /// the already-negotiated scale.
    pub fn set_receive_window_limit(&mut self, limit: Option<usize>) {
        self.receive_window_limit = limit;
        if matches!(self.state, State::Closed | State::Listen) {
            self.remote_win_shift = Self::window_scale_for(self.max_receive_window());
        }
    }
    ...
    pub(crate) fn process(
        ...
        let window_end = window_start + self.receive_window();
        ...
        F: FnOnce(&mut Context, (IpRepr, TcpRepr)) -> Result<(), E>,
    {
        ...
        match self.state {
            ...
            State::SynSent | State::SynReceived => {
                ...
                repr.window_len = self.receive_window().min((1 << 16) - 1) as u16;
...
mod test {
    ...
    macro_rules! sanity {
        ($socket1:expr, $socket2:expr) => {{
            ...
            assert_eq!(
                s1.receive_window_limit, s2.receive_window_limit,
    ...
    #[test]
    fn receive_window_limit_is_independent_from_storage_capacity() {
        const STORAGE_BYTES: usize = 1024 * 1024;
        const WINDOW_LIMIT_BYTES: usize = 368_640;

        let mut s = socket_with_buffer_sizes(64, STORAGE_BYTES);
        s.set_receive_window_limit(Some(WINDOW_LIMIT_BYTES));
        s.state = State::Established;

        assert_eq!(s.recv_capacity(), STORAGE_BYTES);
        assert_eq!(s.receive_window_limit(), Some(WINDOW_LIMIT_BYTES));
        assert_eq!(s.remote_win_shift, 3);
        assert_eq!(
            (s.scaled_window() as usize) << s.remote_win_shift as usize,
            WINDOW_LIMIT_BYTES
        );
    ...
    #[test]
    fn receive_window_limit_subtracts_unconsumed_queue_bytes() {
        const STORAGE_BYTES: usize = 1024 * 1024;
        const WINDOW_LIMIT_BYTES: usize = 368_640;
        const QUEUED_BYTES: usize = 100_000;

        let mut s = socket_with_buffer_sizes(64, STORAGE_BYTES);
        s.set_receive_window_limit(Some(WINDOW_LIMIT_BYTES));
        s.state = State::Established;
        assert_eq!(
            s.rx_buffer.enqueue_slice(&vec![0; QUEUED_BYTES]),
            QUEUED_BYTES
        );
        assert_eq!(s.recv_queue(), QUEUED_BYTES);
        assert_eq!(
            (s.scaled_window() as usize) << s.remote_win_shift as usize,
            WINDOW_LIMIT_BYTES - QUEUED_BYTES
        );

        let mut established = socket_established_with_buffer_sizes(64, STORAGE_BYTES);
        established.state = State::Closed;
        established.set_receive_window_limit(Some(WINDOW_LIMIT_BYTES));
        established.state = State::Established;
        assert_eq!(
            established
                .rx_buffer
                .enqueue_slice(&vec![0; QUEUED_BYTES]),
            QUEUED_BYTES
        );
        send!(
            established,
            TcpRepr {
                seq_number: REMOTE_SEQ + 1 + WINDOW_LIMIT_BYTES,
                ack_number: Some(LOCAL_SEQ + 1),
                ..SEND_TEMPL
            },
            Some(TcpRepr {
                seq_number: LOCAL_SEQ + 1,
                ack_number: Some(REMOTE_SEQ + 1 + QUEUED_BYTES),
                window_len: ((WINDOW_LIMIT_BYTES - QUEUED_BYTES) >> 3) as u16,
                ..RECV_TEMPL
            })
        );
        assert_eq!(established.recv_queue(), QUEUED_BYTES);
    }

    #[test]
    fn receive_window_limit_controls_syn_and_segment_acceptability() {
        const STORAGE_BYTES: usize = 1024 * 1024;
        const WINDOW_LIMIT_BYTES: usize = 368_640;

        let mut listener = socket_with_buffer_sizes(64, STORAGE_BYTES);
        listener.set_receive_window_limit(Some(WINDOW_LIMIT_BYTES));
        ...
        send!(
            ...
            Some(TcpRepr {
                seq_number: LOCAL_SEQ + 1,
                ack_number: Some(REMOTE_SEQ + 1),
                window_len: (WINDOW_LIMIT_BYTES >> 3) as u16,
                ..RECV_TEMPL
            })
        );
        assert_eq!(established.recv_queue(), 0);
    }

    #[test]
    fn default_receive_window_still_uses_physical_storage() {
        const STORAGE_BYTES: usize = 1024 * 1024;

        let mut s = socket_with_buffer_sizes(64, STORAGE_BYTES);
        assert_eq!(s.receive_window_limit(), None);
        ...
        s.set_receive_window_limit(Some(368_640));
        assert_eq!(s.remote_win_shift, 3);
        s.set_receive_window_limit(None);
        assert_eq!(s.receive_window_limit(), None);
        assert_eq!(s.remote_win_shift, 5);
        assert_eq!(
            (s.scaled_window() as usize) << s.remote_win_shift as usize,
