...
pub struct PathStats {
    ...
    pub current_mtu: u16,
    /// Pacing capacity Quinn would use without an application-configured burst cap
    pub pacing_uncapped_capacity_bytes: u64,
    /// Effective current pacing capacity
    pub pacing_capacity_bytes: u64,
    /// Tokens currently stored in the pacing bucket
    pub pacing_tokens_bytes: u64,
    /// MTU used for the current pacing capacity and token accounting
    pub pacing_mtu: u16,
    /// Whether an application-configured maximum currently lowers pacing capacity
    pub pacing_cap_active: bool,
    /// Number of times pacing returned a future send deadline
    pub pacing_delay_events: u64,
}
