...
impl PathData {
    pub(super) fn new(
        ...
        Self {
            ...
            sending_ecn: true,
            pacing: Pacer::new_configured(
                config.initial_rtt,
                congestion.initial_window(),
                config.get_initial_mtu(),
                now,
                config.max_pacing_burst_datagram_equivalents,
            ),
            congestion,
            challenge: None,
    ...
    pub(super) fn from_previous(
        ...
        Self {
            ...
            pacing: Pacer::new_configured(
                ...
                prev.current_mtu(),
                now,
                prev.pacing.max_burst_datagram_equivalents(),
            ),
            sending_ecn: true,
            congestion,
            challenge: None,
...
mod tests {
    ...
    use super::PathData;
    use crate::{Duration, Instant, TransportConfig};

    #[test]
    fn transport_config_reaches_new_path_pacer() {
        let mut config = TransportConfig::default();
        config.initial_rtt(Duration::ZERO);
        config.max_pacing_burst_datagram_equivalents(NonZeroU16::new(64));
        ...
        let mtu = u64::from(path.current_mtu());

        assert_eq!(path.pacing.uncapped_capacity(), 256 * mtu);
        assert_eq!(path.pacing.capacity(), 64 * mtu);
        assert_eq!(path.pacing.tokens(), 64 * mtu);
        assert!(path.pacing.configured_cap_active());
        assert_eq!(path.pacing.delay_events(), 0);
    }

    #[test]
    fn migrated_path_inherits_configured_pacing_cap() {
        let mut config = TransportConfig::default();
        config.initial_rtt(Duration::ZERO);
        config.max_pacing_burst_datagram_equivalents(NonZeroU16::new(64));
        let now = Instant::now();
        let original = PathData::new(
            "127.0.0.1:4433".parse().unwrap(),
            false,
            None,
            0,
        ...
        let migrated = PathData::from_previous(
            "127.0.0.2:4433".parse().unwrap(),
            &original,
            1,
            now + Duration::from_millis(1),
        );
        let mtu = u64::from(migrated.current_mtu());

        assert_eq!(migrated.pacing.capacity(), 64 * mtu);
        assert!(migrated.pacing.configured_cap_active());
    }
}
