...
pub(crate) mod endpoint_pacing;
use endpoint_pacing::{
    ConnectionEndpointPacing, DatagramReservation, PacingTrafficClass, PollReservation,
};
#[doc(hidden)]
pub use endpoint_pacing::{
    EndpointPacingConnectionSnapshot, EndpointPacingSnapshot,
    SocketOutcomeError as EndpointPacingSocketOutcomeError,
    StatelessResponseDenied as EndpointPacingStatelessResponseDenied,
    StatelessResponseReservation as EndpointPacingStatelessResponseReservation,
};
...
pub use streams::{
    Chunks, ClosedStream, FinishError, ReadError, ReadableError, RecvStream, RecvStreamProgress,
    SendStream, SendStreamProgress, ShouldTransmit, StreamEvent, Streams, WriteError, Written,
};
...
pub struct Connection {
    ...
    endpoint_pacing: Option<ConnectionEndpointPacing>,
...
impl Connection {
    ...
    #[cfg(test)]
    pub(crate) fn endpoint_pacing_service_id(&self) -> Option<usize> {
        self.endpoint_pacing
            .as_ref()
            .map(ConnectionEndpointPacing::service_id)
    }

    /// Return bytes in the one exposed UDP socket batch, when endpoint pacing is active.
    #[doc(hidden)]
    pub fn endpoint_pacing_pending_socket_batch_bytes(&self) -> Option<u64> {
        self.endpoint_pacing
            .as_ref()
            .map(ConnectionEndpointPacing::pending_socket_batch_bytes)
    }

    /// Return this connection's endpoint pacing service counters when configured.
    #[doc(hidden)]
    pub fn endpoint_pacing_connection_snapshot(&self) -> Option<EndpointPacingConnectionSnapshot> {
        self.endpoint_pacing
            .as_ref()
            .and_then(ConnectionEndpointPacing::snapshot)
    }

    /// Whether the optional endpoint pacing driver adapter is active.
    #[doc(hidden)]
    pub fn endpoint_pacing_active(&self) -> bool {
        self.endpoint_pacing.is_some()
    }

    /// Supply the current connection-driver waker to the endpoint pacing service.
    #[doc(hidden)]
    pub fn endpoint_pacing_set_driver_waker(&mut self, waker: &Waker) {
        if let Some(pacing) = self.endpoint_pacing.as_mut() {
            pacing.set_driver_waker(waker);
        }
    }

    /// Report successful acceptance of the pending UDP socket batch.
    #[doc(hidden)]
    pub fn endpoint_pacing_note_socket_sent(
        &mut self,
        now: Instant,
    ) -> Result<bool, EndpointPacingSocketOutcomeError> {
        self.endpoint_pacing
            .as_mut()
            .map_or(Ok(false), |pacing| pacing.note_socket_sent(now))
    }

    /// Report that the charged UDP socket batch remains outstanding after `WouldBlock`.
    #[doc(hidden)]
    pub fn endpoint_pacing_note_socket_would_block(
        &self,
    ) -> Result<bool, EndpointPacingSocketOutcomeError> {
        self.endpoint_pacing
            .as_ref()
            .map_or(Ok(false), ConnectionEndpointPacing::note_socket_would_block)
    }

    /// Report abandonment of the pending UDP socket batch.
    #[doc(hidden)]
    pub fn endpoint_pacing_note_socket_abandoned(
        &mut self,
        now: Instant,
    ) -> Result<bool, EndpointPacingSocketOutcomeError> {
        self.endpoint_pacing
            .as_mut()
            .map_or(Ok(false), |pacing| pacing.note_socket_abandoned(now))
    }

    /// Detach endpoint pacing state when the external connection driver is torn down.
    #[doc(hidden)]
    pub fn endpoint_pacing_detach(&mut self, now: Instant) -> bool {
        let Some(mut pacing) = self.endpoint_pacing.take() else {
            return false;
        };
        let detached = pacing.detach(now).is_ok();
        debug_assert!(
            detached,
            "endpoint pacing detach must match a live connection"
        );
        detached
    }

    fn endpoint_pacing_begin_transmit_poll(&self) -> bool {
        self.endpoint_pacing
            .as_ref()
            .is_none_or(ConnectionEndpointPacing::begin_transmit_poll)
    }

    fn endpoint_pacing_reserve_datagram(
        &mut self,
        class: PacingTrafficClass,
        path_ready_at: Instant,
        planned_bytes: usize,
        now: Instant,
    ) -> Result<Option<DatagramReservation>, ()> {
        let Some(pacing) = self.endpoint_pacing.as_mut() else {
            return Ok(None);
        };
        let result = pacing.poll_reserve(class, path_ready_at, planned_bytes as u64, now);
        match result {
            PollReservation::Granted(reservation) => Ok(Some(reservation)),
            PollReservation::Blocked { deadline, reason } => {
                trace!(?reason, "endpoint pacing reservation blocked");
    ...
    fn endpoint_pacing_settle_datagram(
        &mut self,
        reservation: Option<DatagramReservation>,
        actual_bytes: usize,
    ) -> bool {
        let Some(reservation) = reservation else {
            return true;
        };
        let settled = self.endpoint_pacing.as_mut().is_some_and(|pacing| {
            pacing
                .settle_datagram(reservation, actual_bytes as u64)
                .is_ok()
        });
        debug_assert!(settled, "built datagram must fit its endpoint reservation");
        settled
    }

    fn endpoint_pacing_expose_socket_batch(&mut self) -> bool {
        self.endpoint_pacing
            .as_mut()
            .is_none_or(ConnectionEndpointPacing::expose_socket_batch)
    }

    fn endpoint_pacing_cancel_waiter(&self) {
        if let Some(pacing) = self.endpoint_pacing.as_ref() {
            pacing.cancel_waiter();
        }
    }
    ...
    pub fn poll_transmit(
        ...
        if !self.endpoint_pacing_begin_transmit_poll() {
            return None;
        }
        ...
        match self.send_path_challenge(now, buf) {
            Ok(Some(challenge)) => return Some(challenge),
            Ok(None) => {}
            Err(()) => return None,
        }
        ...
        let close = match self.state {
            State::Drained => {
                ...
                self.endpoint_pacing_cancel_waiter();
                return None;
            }
        ...
        while space_idx < spaces.len() {
            ...
            if !coalesce || buf_capacity - buf_end < MIN_PACKET_SPACE + tag_len {
                ...
                if ack_eliciting && self.spaces[space_id].loss_probes == 0 {
                    ...
                    if let Some(delay) = self.path.pacing.delay(
                        ...
                        if builder_storage.is_none() && self.endpoint_pacing_active() {
                            let planned_bytes = match self.spaces[space_id].loss_probes {
                                0 => segment_size,
                                _ => std::cmp::min(segment_size, usize::from(INITIAL_MTU)),
                            };
                            let class =
                                self.endpoint_pacing_class(space_id, close, frame_space_1rtt);
                            endpoint_blocked = self
                                .endpoint_pacing_reserve_datagram(class, delay, planned_bytes, now)
                                .is_err();
                        }
                        congestion_blocked = true;
                ...
                if let Some(mut builder) = builder_storage.take() {
                    ...
                    let datagram_bytes = buf.len() - datagram_start;
                    if !self.endpoint_pacing_settle_datagram(
                        endpoint_reservation.take(),
                        datagram_bytes,
                    ) {
                        return None;
                    }
                ...
                let loss_probe = self.spaces[space_id].loss_probes != 0;
                let next_datagram_size_limit = if loss_probe {
                ...
                let class = self.endpoint_pacing_class(space_id, close, frame_space_1rtt);
                endpoint_reservation = match self.endpoint_pacing_reserve_datagram(
                    class,
                    now,
                    next_datagram_size_limit,
                    now,
                ) {
            ...
            if space_id == SpaceId::Data && num_datagrams == 1 {
                if let Some((token, remote)) = self.path_responses.pop_off_path(self.path.remote) {
                    ...
                    self.endpoint_pacing_cancel_waiter();
                    if !self.endpoint_pacing_expose_socket_batch() {
                        return None;
                    }
        ...
        if let Some(mut builder) = builder_storage {
            ...
            let datagram_bytes = buf.len() - datagram_start;
            if !self.endpoint_pacing_settle_datagram(endpoint_reservation.take(), datagram_bytes) {
                return None;
            }
            self.path
                .congestion
        ...
        if buf.is_empty() && self.state.is_established() {
            ...
            let mut next_mtud = self.path.mtud.clone();
            let Some(probe_size) = next_mtud
                .poll_transmit(now, self.packet_number_filter.peek(&self.spaces[space_id]))
            else {
                self.path.mtud = next_mtud;
                if !endpoint_blocked {
                    self.endpoint_pacing_cancel_waiter();
                }
                return None;
            ...
            if !self.endpoint_pacing_settle_datagram(endpoint_reservation.take(), buf.len()) {
                return None;
            }
    ...
    fn send_path_challenge(
        ...
        let Some((prev_cid, prev_path)) = self.prev_path.as_ref() else {
            return Ok(None);
        };
        ...
        let prev_cid = *prev_cid;
        let reservation = self.endpoint_pacing_reserve_datagram(
            PacingTrafficClass::Control,
            now,
        ...
        self.prev_path
            .as_mut()
            .expect("previous path remained present during reservation")
            .1
        ...
        if !self.endpoint_pacing_settle_datagram(reservation, buf.len()) {
            return Ok(None);
        }
        self.stats.udp_tx.on_sent(1, buf.len());
    ...
    fn endpoint_pacing_class(
        &self,
        space_id: SpaceId,
        close: bool,
        frame_space_1rtt: usize,
    ) -> PacingTrafficClass {
        let application_datagram_ready = self
            .datagrams
            .outgoing
            .front()
            .is_some_and(|datagram| datagram.size(true) <= frame_space_1rtt);
    ...
    pub fn stats(&self) -> ConnectionStats {
        ...
        stats.path.pacing_uncapped_capacity_bytes = self.path.pacing.uncapped_capacity();
        stats.path.pacing_capacity_bytes = self.path.pacing.capacity();
        stats.path.pacing_tokens_bytes = self.path.pacing.tokens();
        stats.path.pacing_mtu = self.path.pacing.mtu();
        stats.path.pacing_cap_active = self.path.pacing.configured_cap_active();
        stats.path.pacing_delay_events = self.path.pacing.delay_events();
    ...
    fn migrate(&mut self, now: Instant, remote: SocketAddr) {
        ...
        if let Some(pacing) = self.endpoint_pacing.as_mut() {
            let pacing_migrated = pacing.migrate(self.path_counter).is_ok();
            debug_assert!(
                pacing_migrated,
                "endpoint pacing path generation must be unique"
            );
        }
...
fn classify_endpoint_pacing_datagram(
    space_id: SpaceId,
    close: bool,
    loss_probe: bool,
    application_payload_ready: bool,
) -> PacingTrafficClass {
    if space_id != SpaceId::Data || close || loss_probe {
        PacingTrafficClass::Control
    } else if application_payload_ready {
...
mod tests {
    ...
    #[test]
    fn endpoint_pacing_classifies_protocol_control_and_application_payload() {
        for space in [SpaceId::Initial, SpaceId::Handshake] {
            assert_eq!(
                classify_endpoint_pacing_datagram(space, false, false, false),
                PacingTrafficClass::Control
            );
            assert_eq!(
                classify_endpoint_pacing_datagram(space, false, false, true),
                PacingTrafficClass::Control,
                "application payload is impossible before the data space"
            );
        }

        for (close, loss_probe) in [(true, false), (false, true), (true, true)] {
            assert_eq!(
                classify_endpoint_pacing_datagram(SpaceId::Data, close, loss_probe, false),
                PacingTrafficClass::Control
            );
            assert_eq!(
                classify_endpoint_pacing_datagram(SpaceId::Data, close, loss_probe, true),
                PacingTrafficClass::Control,
                "close and loss probes retain control classification"
            );
        }

        assert_eq!(
            classify_endpoint_pacing_datagram(SpaceId::Data, false, false, false),
            PacingTrafficClass::Control,
            "ACK, keepalive, path, MTU, and flow-control-only packets are control"
        );
        assert_eq!(
            classify_endpoint_pacing_datagram(SpaceId::Data, false, false, true),
            PacingTrafficClass::Bulk,
