...
#[test]
fn configured_pacing_cap_is_visible_in_connection_stats() {
    let mut config = client_config();
    let transport = Arc::get_mut(&mut config.transport).unwrap();
    transport.initial_rtt(Duration::from_micros(1));
    transport.max_pacing_burst_datagram_equivalents(std::num::NonZeroU16::new(64));

    let mut pair = Pair::default();
    let (client_ch, _) = pair.connect_with(config);
    let path = pair.client_conn_mut(client_ch).stats().path;
    let pacing_mtu = u64::from(path.pacing_mtu);

    assert_eq!(path.pacing_uncapped_capacity_bytes, 256 * pacing_mtu);
    assert_eq!(path.pacing_capacity_bytes, 64 * pacing_mtu);
    assert!(path.pacing_tokens_bytes <= path.pacing_capacity_bytes);
    assert!(path.pacing_cap_active);
    assert!(path.pacing_mtu <= path.current_mtu);
}

#[test]
fn endpoint_pacing_service_is_shared_per_endpoint_and_default_off() {
    let mut default_pair = Pair::default();
    let (default_client, _) = default_pair.connect();
    assert_eq!(
        default_pair
            .client_conn_mut(default_client)
            .endpoint_pacing_service_id(),
        None
    );
    ...
    let mut first_pair = Pair::new(endpoint_config.clone(), server_config());
    let (first_client, first_server) = first_pair.connect();
    let second_client = first_pair.begin_connect(client_config());
    let first_client_service = first_pair
        .client_conn_mut(first_client)
        .endpoint_pacing_service_id()
        .expect("configured client endpoint service");
    let second_client_service = first_pair
        .client_conn_mut(second_client)
        .endpoint_pacing_service_id()
        .expect("second client connection service");
    let first_server_service = first_pair
        .server_conn_mut(first_server)
        .endpoint_pacing_service_id()
        .expect("configured server endpoint service");

    assert_eq!(first_client_service, second_client_service);
    assert_ne!(first_client_service, first_server_service);

    let mut second_pair = Pair::new(endpoint_config, server_config());
    let third_client = second_pair.begin_connect(client_config());
    let third_client_service = second_pair
        .client_conn_mut(third_client)
        .endpoint_pacing_service_id()
        .expect("fresh configured client endpoint service");
    assert_ne!(first_client_service, third_client_service);
}
...
#[test]
fn endpoint_pacing_preaccounts_every_stream_gso_datagram_and_batch() {
    let _guard = subscribe();
    let mut endpoint_config = EndpointConfig::default();
    endpoint_config
        .endpoint_pacing_service(Some(EndpointPacingServiceConfig::endpoint_window_v1()));
    ...
    let initial = pair.client_conn_mut(client_ch).stats().udp_tx;
    let pacing_initial = pair
        .client
        .endpoint
        .endpoint_pacing_snapshot()
        .expect("configured client endpoint must expose pacing counters");
    ...
    let final_stats = pair.client_conn_mut(client_ch).stats().udp_tx;
    let pacing_final = pair
        .client
        .endpoint
        .endpoint_pacing_snapshot()
        .expect("configured client endpoint must retain pacing counters");
    assert!(
        final_stats.datagrams - initial.datagrams > final_stats.ios - initial.ios,
        "configured stream data must retain GSO"
    );
    assert_eq!(final_stats.ios - initial.ios, 2);
    assert_eq!(
        pacing_final.sent_datagrams - pacing_initial.sent_datagrams,
        final_stats.datagrams - initial.datagrams,
        "each GSO segment must retain an individual endpoint service charge"
    );
    assert_eq!(
        pacing_final.sent_bytes - pacing_initial.sent_bytes,
        final_stats.bytes - initial.bytes,
        "endpoint socket outcomes must match Quinn UDP accounting"
    );
    assert_eq!(pacing_final.live_reservation_bytes, 0);
    assert_eq!(pacing_final.outstanding_bytes, 0);
    assert_eq!(
        pair.client_conn_mut(client_ch)
...
fn configured_pacing_cap_accounts_bulk_datagrams_before_gso_io() {
    ...
    let (client_ch, _) = pair.connect_with(config);
    let stream = pair.client_streams(client_ch).open(Dir::Uni).unwrap();
    let initial = pair.client_conn_mut(client_ch).stats();
    ...
    let final_stats = pair.client_conn_mut(client_ch).stats();
    let datagrams = final_stats.udp_tx.datagrams - initial.udp_tx.datagrams;
    let ios = final_stats.udp_tx.ios - initial.udp_tx.ios;
    let paced_bytes = initial.path.pacing_tokens_bytes - final_stats.path.pacing_tokens_bytes;

    assert!(datagrams > ios, "GSO must combine multiple paced datagrams");
    assert_eq!(ios, 2);
    assert!(paced_bytes >= 20 * 1024);
    assert!(final_stats.path.pacing_cap_active);
    assert_eq!(
        final_stats.path.pacing_capacity_bytes,
        64 * u64::from(final_stats.path.pacing_mtu)
    );
}
