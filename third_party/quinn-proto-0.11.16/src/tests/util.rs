        ...
        loop {
            ...
            for (ch, conn) in self.connections.iter_mut() {
                ...
                while let Some(transmit) = conn.poll_transmit(now, MAX_DATAGRAMS, &mut buf) {
                    let size = transmit.size;
                    if let Some(pending_bytes) = conn.endpoint_pacing_pending_socket_batch_bytes() {
                        assert_eq!(
                            pending_bytes, size as u64,
                            "every configured Transmit must expose its exact socket batch"
                        );
                    }
                    self.outbound.extend(split_transmit(transmit, &buf[..size]));
                    buf.clear();
                    conn.endpoint_pacing_note_socket_sent(now)
                        .expect("test endpoint accepts the complete socket batch");
                    assert!(
                        conn.endpoint_pacing_pending_socket_batch_bytes()
                            .is_none_or(|pending_bytes| pending_bytes == 0),
                        "socket success must clear the configured pending batch"
                    );
                }
