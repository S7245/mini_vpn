...
pub struct EndpointConfig {
    ...
    pub(crate) endpoint_pacing_service: Option<EndpointPacingServiceConfig>,
}

/// Static parameters for one endpoint-owned datagram pacing service
///
/// Use [`Self::endpoint_window_v1`] for the fixed mini_vpn candidate. The
/// individual fields are readable for validation and startup attribution, but
/// arbitrary construction remains internal while the policy is experimental.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct EndpointPacingServiceConfig {
    pub(crate) rate_bytes_per_second: u64,
    pub(crate) burst_bytes: u64,
    pub(crate) control_reserve_bytes: u64,
    pub(crate) connection_quantum_bytes: u64,
}

impl EndpointPacingServiceConfig {
    /// Construct the fixed first endpoint-window implementation.
    #[must_use]
    pub fn endpoint_window_v1() -> Self {
        Self {
            rate_bytes_per_second: 30_720_000,
            burst_bytes: 61_440,
            control_reserve_bytes: 10_240,
            connection_quantum_bytes: 20_480,
        }
    }
    ...
    pub(crate) fn new(
        ...
        connection_quantum_bytes: u64,
    ) -> Result<Self, ConfigError> {
        if rate_bytes_per_second == 0
            || burst_bytes == 0
            || control_reserve_bytes > burst_bytes
            || connection_quantum_bytes == 0
            || connection_quantum_bytes > burst_bytes
        {
    ...
    /// Aggregate sustained wire-byte rate.
    #[must_use]
    pub fn rate_bytes_per_second(self) -> u64 {
        self.rate_bytes_per_second
    }

    /// Aggregate wire-byte burst capacity.
    #[must_use]
    pub fn burst_bytes(self) -> u64 {
        self.burst_bytes
    }

    /// Control-class reserve contained within the aggregate burst.
    #[must_use]
    pub fn control_reserve_bytes(self) -> u64 {
        self.control_reserve_bytes
    }

    /// Per-connection bulk service quantum.
    #[must_use]
    pub fn connection_quantum_bytes(self) -> u64 {
        self.connection_quantum_bytes
    }
}
...
impl EndpointConfig {
    ...
    /// Configure one optional byte pacing service shared by all connections
    /// created by this endpoint.
    ///
    /// `None`, the default, preserves Quinn's upstream connection driver and
    /// per-path pacing behavior.
    pub fn endpoint_pacing_service(
        &mut self,
        config: Option<EndpointPacingServiceConfig>,
    ) -> &mut Self {
        self.endpoint_pacing_service = config;
        self
    }
}
...
impl fmt::Debug for EndpointConfig {
    fn fmt(&self, fmt: &mut fmt::Formatter<'_>) -> fmt::Result {
        let mut debug = fmt.debug_struct("EndpointConfig");
        debug
            // reset_key not debug
        ...
        if let Some(config) = self.endpoint_pacing_service {
            debug.field("endpoint_pacing_service", &config);
        }
...
#[cfg(test)]
mod endpoint_pacing_tests {
    use super::{ConfigError, EndpointConfig, EndpointPacingServiceConfig};

    #[test]
    fn endpoint_window_v1_uses_the_frozen_candidate() {
        let config = EndpointPacingServiceConfig::endpoint_window_v1();
        assert_eq!(config.rate_bytes_per_second(), 30_720_000);
        assert_eq!(config.burst_bytes(), 61_440);
        assert_eq!(config.control_reserve_bytes(), 10_240);
        assert_eq!(config.connection_quantum_bytes(), 20_480);
    }

    #[test]
    fn endpoint_pacing_config_rejects_invalid_bounds() {
        assert_eq!(
            EndpointPacingServiceConfig::new(0, 1, 0, 1),
            Err(ConfigError::OutOfBounds)
        );
        assert_eq!(
            EndpointPacingServiceConfig::new(1, 0, 0, 1),
            Err(ConfigError::OutOfBounds)
        );
        assert_eq!(
            EndpointPacingServiceConfig::new(1, 10, 11, 1),
            Err(ConfigError::OutOfBounds)
        );
        assert_eq!(
            EndpointPacingServiceConfig::new(1, 10, 0, 0),
            Err(ConfigError::OutOfBounds)
        );
        assert_eq!(
            EndpointPacingServiceConfig::new(1, 10, 0, 11),
            Err(ConfigError::OutOfBounds)
        );
    }

    #[test]
    fn default_endpoint_config_is_identical_to_explicit_none() {
        let default = EndpointConfig::default();
        let default_debug = format!("{default:?}");
        assert!(!default_debug.contains("endpoint_pacing_service"));

        let mut explicit_none = default.clone();
        explicit_none.endpoint_pacing_service(None);
        assert_eq!(format!("{explicit_none:?}"), default_debug);

        let mut configured = default;
        configured.endpoint_pacing_service(Some(EndpointPacingServiceConfig::endpoint_window_v1()));
        assert!(format!("{configured:?}").contains("endpoint_pacing_service"));
    }
}
