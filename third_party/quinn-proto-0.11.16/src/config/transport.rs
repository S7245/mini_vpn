...
pub struct TransportConfig {
    ...
    pub(crate) max_pacing_burst_datagram_equivalents: Option<NonZeroU16>,
...
impl TransportConfig {
    ...
    /// Maximum amount of paced data one connection may accumulate, measured in current-MTU
    /// datagram equivalents.
    ///
    /// `None`, the default, preserves Quinn's upstream maximum of 256 MTU equivalents. Values
    /// above 256 cannot increase that upstream maximum. Values below 10 also lower Quinn's usual
    /// minimum burst so the configured maximum remains authoritative.
    pub fn max_pacing_burst_datagram_equivalents(
        &mut self,
        value: Option<NonZeroU16>,
    ) -> &mut Self {
        self.max_pacing_burst_datagram_equivalents = value;
        self
    }
...
impl Default for TransportConfig {
    fn default() -> Self {
        ...
        Self {
            ...
            max_pacing_burst_datagram_equivalents: None,
...
impl fmt::Debug for TransportConfig {
    fn fmt(&self, fmt: &mut fmt::Formatter<'_>) -> fmt::Result {
        let Self {
            ...
            max_pacing_burst_datagram_equivalents,
            qlog_sink,
        } = self;
        ...
        if let Some(value) = max_pacing_burst_datagram_equivalents {
            s.field("max_pacing_burst_datagram_equivalents", &value.get());
        }
        if cfg!(feature = "qlog") {
...
mod tests {
    ...
    #[test]
    fn pacing_burst_cap_is_default_off_and_configurable() {
        let default = TransportConfig::default();
        assert!(!format!("{default:?}").contains("max_pacing_burst_datagram_equivalents"));

        let mut capped = TransportConfig::default();
        capped.max_pacing_burst_datagram_equivalents(NonZeroU16::new(64));
        assert!(format!("{capped:?}").contains("max_pacing_burst_datagram_equivalents: 64"));
    }
}
