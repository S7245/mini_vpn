...
run_lowrtt_probe() {
  ...
  if [[ -f "$probe_out" ]]; then
    append '```text'
    grep -E 'Attribution Summary|iperf_sender_mbps|iperf_receiver_mbps|tcp_pool:|local_write_pressure:|global_rx_pressure:|downlink_backpressure:|downlink_flush:|relay_late_remote:|tun_drops:|quic:|attribution:|local 10[.]0[.]0[.]1|receiver$|sender$|error -|Connection reset|log not found|📊|🔬|TUIC datagram|UDP relay mode|tuic-tcp-pool-reconnect|tcp-(relay-live|relay-write-half-closed|relay-close|handle-close|local-write-pressure|global-rx-pressure|downlink-backpressure|downlink-flush|loop-flush-tx|tun-flush-fail|send-slice-error)|exit=' "$probe_out" | tail -200 | tee -a "$REPORT" || true
    append '```'
  else
