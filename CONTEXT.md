...
**TUIC TCP ordered-gap path migration**:
One bounded active-migration authority for a live D16 ordered receive stream. Quinn must expose that the application-consumed prefix has stopped at an exact stream offset while bytes strictly beyond the missing prefix remain buffered for two consecutive existing recovery samples. The client then rebinds the shared Endpoint UDP socket once, preserving the QUIC identity, TUIC stream, Target TCP connection, and payload bytes so the peer validates a fresh path. Progress, stream release, or connection-generation replacement begins observation again; ordinary TCP silence and connection-wide RX counters are never sufficient authority.
_Avoid_: read timeout (there is no new duration knob), stream replay (payload and stream identity are retained), client path reset (migration must also change the peer's sending path state), generic no-RX recovery (legitimate idle TCP flows remain ineligible)
