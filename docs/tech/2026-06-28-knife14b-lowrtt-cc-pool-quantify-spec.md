...
先跑 direct baseline，分别证明 `client→exit` 与 `exit→iperf target` 裸路径本身是否足够胖：
...
- `scripts/knife14b-direct-baseline.sh`：收集 direct path baseline，验证 `client→exit` / `exit→target` 裸路径是否 `>=150M`。
- 本 spec + plan。
