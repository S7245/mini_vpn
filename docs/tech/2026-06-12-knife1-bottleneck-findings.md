...
# 刀5 — 拦全 :53 裸包 DNS 劫持(2026-06-22)

> spec/plan：`2026-06-22-knife5-dns-hijack-{spec,plan}.md`；ADR `docs/adr/0007-hijack-all-plaintext-dns.md`。
> 分支 `claude/knife5-dns-hijack`(从 main 起,逐 commit push)。补上刀4 deferred 的「拦全:53」——
> 应用回落明文 DNS 时查的是**它自己配的 resolver**(非 198.18.0.1),原先被隧道转发到真 DNS → 真实 IP 绕过
> fake-IP。本刀让**任意 resolver 的明文 :53 都本地伪造 fake-IP**(裸包构造,src=被查询的 resolver),
> 废 smoltcp DNS socket、统一一条路径,fake-IP 路由不再依赖系统 DNS 指向我方。

## 真出口 acceptance 配方(soak-knife5)
```bash
export MINI_VPN_TUIC_SERVER=<VPS_IP>:8443 MINI_VPN_TUIC_UUID=<uuid> MINI_VPN_TUIC_PASSWORD=<pass>
cargo build --release
sudo -E bash scripts/knife35-acceptance.sh soak-knife5      # DNS=8.8.8.8(非我方 resolver)+ 路由进 TUN + /15
# 另开终端：
dig @8.8.8.8 example.com +short            # 期望 198.18.x.x(fake-IP,非真实 IP)
dig +tcp @8.8.8.8 example.com +short       # 期望 connection reset(TCP :53 RST,无 IP)
curl -sI https://example.com | head -1     # 期望 HTTP/.. 200
for d in www.google.com github.com www.cloudflare.com; do dig @8.8.8.8 "$d" +short|head -1; done
grep '🪪 DNS' /tmp/mvpn_accept.log | tail   # 期望 见域名 → fake-IP
sudo -E bash scripts/knife35-acceptance.sh soak-stop       # 还原 DNS + 清路由
```

## 实测 / 裁决(2026-06-22,测试机,native+cubic 全局隧道,**系统 DNS=8.8.8.8**)

**K5-1 任意 resolver :53 劫持 ✅(核心)**:`dig @8.8.8.8 example.com` → `198.18.0.36`(fake-IP)。系统 DNS
设成 **8.8.8.8(非 198.18.0.1)**,明文 :53 仍被本地伪造 → **北极星达成:fake-IP 不再依赖系统 DNS 指向我方 resolver**。

**K5-2 TCP :53 RST ✅**:`dig +tcp @8.8.8.8 example.com` → `communications error ... connection reset`、**无 IP 返回**
→ `resolve_target` 的 `is_dns_relay_port(53)` Block → rearm RST 命中,无 real-IP 泄漏。(dig `rc=0` 是其退出码怪癖,
行为证据是 reset+无应答。)不变量成立:UDP :53 已被 classify 截走,`port==53` 只命中 TCP。

**K5-3 经隧道上网 ✅**:`curl -sI https://example.com` → `HTTP/2 200`(fake-IP→DomainPort→隧道)。

**K5-4 多域名零逃逸 ✅**:google/github/cloudflare → `198.18.0.46/68/75`,全 fake-IP、每域名稳定独立 IP,无真实 IP 泄漏。

**K5-5 真实 app 流量 ✅**:日志见 apple/icloud/marscode/google 等 `🪪 DNS → fake-IP`,劫持路径真生效。

**刀4↔刀5 链路闭环 ✅**:日志见 `dns.google (A) → fake-IP 198.18.0.2`——明文解析 DoH 主机名被伪造成 fake-IP,
之后该 fake-IP:443 命中刀4 `is_doh_domain` Block。两刀协同:明文 :53 伪造(刀5)+ 加密 DNS 端点拦截(刀4)。

### 裁决
- **拦全:53 达成** → 刀4 deferred 项 + HANDOFF「拦全:53」已知坑**关闭(✅)**。无缝 on/off「不依赖系统 DNS」拼图就位
  (配合前端 NE)。
- **裸包 vs AnyIP / 废 smoltcp DNS socket / 全劫持 / TCP :53 RST** 四裁决经真出口验证全部成立(见 ADR-0007)。
